...
interface DeleteCertificationConfirmModalProps {
  certification: CertificationEntity | null;
  isOpen: boolean;
  isDeleting: boolean;
  onClose: () => void;
...
export default function DeleteCertificationConfirmModal({ certification, isOpen, isDeleting, onClose, onConfirm }: DeleteCertificationConfirmModalProps) {
      ...
      <div className={`relative w-full max-w-md bg-[#0b121f] border-2 border-rose-500/40 rounded-[2rem] p-6 shadow-[0_0_40px_rgba(244,63,94,0.15)] transition-all duration-300 ${isOpen ? "scale-100 translate-y-0" : "scale-95 translate-y-4"}`}>
        ...
        <div className="flex flex-col items-center text-center pt-4">
          ...
          <p className="text-xs font-mono text-gray-400 mb-6 px-4">
            ...
            <span className="text-rose-400 font-bold uppercase block mt-2">"{certification?.title}"</span> 
          ...
          <div className="flex gap-4 w-full">
            ...
            <button
              type="button"
              onClick={() => certification?.id && onConfirm(certification.id)}
              disabled={isDeleting}
