        ...
        <form action={formAction} className="space-y-6 relative z-10">
          ...
          <div className="space-y-2">
            ...
            {state?.errors?.password && (
              <p className="text-red-500 text-xs mt-1 font-mono flex items-center gap-1">
                <AlertCircle size={12} /> {state.errors.password[0]}
              </p>
            )}
          </div>
          
          {state?.message && !state?.errors?.email && !state?.errors?.password && (
            <p className="text-red-500 text-xs mt-1 font-mono flex items-center gap-1 justify-center p-2 bg-red-500/10 rounded border border-red-500/20">
          ...
          <div className="flex items-center justify-between pt-1">
            <label className="flex items-center gap-3 cursor-pointer group">
              <div className="relative">
                ...
                <div className="w-8 h-4.5 rounded-full transition-colors duration-300 bg-white/5 ring-1 ring-white/10 peer-checked:bg-cyan-500/30 peer-checked:ring-cyan-400/30" />
                <div
                  className="absolute top-[3px] left-[3px] w-2.5 h-2.5 rounded-full bg-gray-400 transition-all duration-300 peer-checked:transform peer-checked:translate-x-3.5 peer-checked:bg-cyan-400 peer-checked:shadow-[0_0_8px_#22d3ee]"
                />
              </div>
          ...
          >
            ...
            {!isPending && <ArrowRight size={14} className="text-white group-hover:translate-x-1 transition-transform duration-300" />}
          </button>
        </form>
      </motion.div>
    </div>
